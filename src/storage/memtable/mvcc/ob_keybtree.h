...
private:
  ...
  // TODO(handora.qc): currently not used
...
// STATIC_ASSERT(sizeof(keybtree::BtreeIterator<ObStoreRowkeyWrapper, ObMvccRow *>) == 4032, "btree iterator size exceeded");
...
public:
  ...
  int get_next(BtreeKey &key, BtreeVal &val);
  // Fetch the key_count and child_count under top_level
  int estimate_key_count(int64_t top_level, int64_t& child_count, int64_t& key_count);
  // split the keybtree range under top_level into key_array according to
  // branch_count(branch keybtree has) and part_count(part user wants to split)
