    ...
    bool exclude_end_key, bool is_backward, std::vector<int64_t> &answer)
{
  ...
  BtreeIterator iter;
  btree->set_key_range(iter, start_key, exclude_start_key, end_key, exclude_end_key, 0);
...
void free_btree(ObKeyBtree &btree)
{
  ...
  BtreeIterator iter;
  btree.set_key_range(iter, start_key, true, end_key, true, 0);
...
TEST(TestEventualConsistency, smoke_test)
{
  ...
  BtreeIterator iter;
  btree.set_key_range(iter, start_key, false, end_key, false, 0);
  int i = 0;
  while (iter.get_next(key, val) == OB_SUCCESS) {
        ...
        [&](int thread_id) {
          ...
          while (scan_count--) {
            ...
            if (thread_id % 2 == 0) {
              ...
              // scan backward
              BtreeIterator iter;
              btree.set_key_range(iter, end_key, false, start_key, false, 0);
