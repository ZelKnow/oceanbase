...
TEST(TestBtree, smoke_test)
{
  ...
  generate_leafnode_keys(data, KEY_NUM);
  ...
  REPEAT_COUNT = 100;
  while (REPEAT_COUNT--) {
    int start_pos = ObRandom::rand(0, KEY_NUM - 1);
    int end_pos = ObRandom::rand(start_pos + 1, KEY_NUM - 1);
    start_key.set_int(data[start_pos]);
    end_key.set_int(data[end_pos]);
    std::vector<int64_t> ans;
